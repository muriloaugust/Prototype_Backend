TAG = latest
DOCKER_LOCAL = Augustos-backend:$(TAG)
DOCKER_REPO = something.dkr.ecr.us-west-2.amazonaws.com/Augustos-backend
...
buildproto:
	augusto_prototype_backend/api/grpc/makeprotobufs

proto: buildproto
	mkdir -p dist && \
	cp -r augusto_prototype_backend/api/grpc/dist/*.tar.gz dist
